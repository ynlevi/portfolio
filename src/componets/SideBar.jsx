...
export default function SideBar({ refs, mainRef, handleClick, isOpen }) {
  return (
    <>
      <AnimatePresence>
        {!isOpen && (
...
const SideMenu = ({ handleClick, refs, mainRef }) => {
  ...
  return (
    <>
      <motion.div
        key={0}
        className="absolute top-0 right-0 w-1/2 z-50 bg-dk-primary  h-screen shadow-2xl flex flex-col justify-between "
        variants={castomSideMenu}
        initial="isClose"
        animate="isOpen"
        exit="isClose"
      >
        <div className=" divide-y divide-dk-secondary ">
